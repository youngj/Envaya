...
class Net_SMTP
{
    ...
    function _send($data)
    {
        ...
        if ($error === false || is_pear_error($error)) {
    ...
    function connect($timeout = null, $persistent = false)
    {
        ...
        if (is_pear_error($error = $this->_parseResponse(220))) {
            return $error;
        }
        ...
        if (is_pear_error($error = $this->_negotiate())) {
            return $error;
        }
    ...
    function disconnect()
    {
        if (is_pear_error($error = $this->_put('QUIT'))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(221))) {
            return $error;
        }
        if (is_pear_error($error = $this->_socket->disconnect())) {
    ...
    function _negotiate()
    {
        if (is_pear_error($error = $this->_put('EHLO', $this->localhost))) {
            return $error;
        }

        if (is_pear_error($this->_parseResponse(250))) {
            ...
            if (is_pear_error($error = $this->_put('HELO', $this->localhost))) {
                return $error;
            }
            if (is_pear_error($this->_parseResponse(250))) {
    ...
    function auth($uid, $pwd , $method = '', $tls = true)
    {
        ...
        if ($tls && version_compare(PHP_VERSION, '5.1.0', '>=') &&
            ...
            if (is_pear_error($result = $this->_put('STARTTLS'))) {
                return $result;
            }
            if (is_pear_error($result = $this->_parseResponse(220))) {
                return $result;
            }
            if (is_pear_error($result = $this->_socket->enableCrypto(true, STREAM_CRYPTO_METHOD_TLS_CLIENT))) {
                return $result;
            } elseif ($result !== true) {
        ...
        if (empty($method)) {
            if (is_pear_error($method = $this->_getBestAuthMethod())) {
    ...
    {
        if (is_pear_error($error = $this->_put('AUTH', 'DIGEST-MD5'))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (is_pear_error($error = $this->_parseResponse(334))) {
        ...
        if (is_pear_error($error = $this->_put($auth_str))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (is_pear_error($error = $this->_parseResponse(334))) {
         ...
         * it. */
        if (is_pear_error($error = $this->_put(''))) {
            return $error;
        }
        /* 235: Authentication successful */
        if (is_pear_error($error = $this->_parseResponse(235))) {
            return $error;
        }
    }
    ...
    {
        if (is_pear_error($error = $this->_put('AUTH', 'CRAM-MD5'))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (is_pear_error($error = $this->_parseResponse(334))) {
    ...
    {
        if (is_pear_error($error = $this->_put('AUTH', 'LOGIN'))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (is_pear_error($error = $this->_parseResponse(334))) {
        ...
        if (is_pear_error($error = $this->_put(base64_encode($uid)))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (is_pear_error($error = $this->_parseResponse(334))) {
        ...
        if (is_pear_error($error = $this->_put(base64_encode($pwd)))) {
            return $error;
        }
    ...
    {
        if (is_pear_error($error = $this->_put('AUTH', 'PLAIN'))) {
            return $error;
        }
        /* 334: Continue authentication request */
        if (is_pear_error($error = $this->_parseResponse(334))) {
    ...
    {
        if (is_pear_error($error = $this->_put('HELO', $domain))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(250))) {
            return $error;
        }
    ...
    function mailFrom($sender, $params = null)
    {
        ...
        if (is_pear_error($error = $this->_put('MAIL', $args))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(250, $this->pipelining))) {
            return $error;
        }
    ...
    function rcptTo($recipient, $params = null)
    {
        ...
        if (is_pear_error($error = $this->_put('RCPT', $args))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(array(250, 251), $this->pipelining))) {
            return $error;
        }
    ...
    function data($data, $headers = null)
    {
        ...
        if (is_pear_error($error = $this->_put('DATA'))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(354))) {
            return $error;
        }
        ...
        if (!is_null($headers)) {
            ...
            if (is_pear_error($result = $this->_send($headers . "\r\n\r\n"))) {
                return $result;
            }
        }
        ...
        if (is_resource($data)) {
            ...
            while ($line = fgets($data, 1024)) {
                $this->quotedata($line);
                if (is_pear_error($result = $this->_send($line))) {
                    return $result;
                }
            }

            /* Finally, send the DATA terminator sequence. */
            if (is_pear_error($result = $this->_send("\r\n.\r\n"))) {
                return $result;
            }
        } else {
             ...
             * terminator. */
            $this->quotedata($data);
            if (is_pear_error($result = $this->_send($data . "\r\n.\r\n"))) {
                return $result;
            }
        }
    ...
    {
        if (is_pear_error($error = $this->_put('SEND', "FROM:<$path>"))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(250, $this->pipelining))) {
            return $error;
    ...
    {
        if (is_pear_error($error = $this->_put('SOML', "FROM:<$path>"))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(250, $this->pipelining))) {
            return $error;
    ...
    {
        if (is_pear_error($error = $this->_put('SAML', "FROM:<$path>"))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(250, $this->pipelining))) {
            return $error;
    ...
    function rset()
    {
        if (is_pear_error($error = $this->_put('RSET'))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(250, $this->pipelining))) {
            return $error;
    ...
    function vrfy($string)
    {
        ...
        if (is_pear_error($error = $this->_put('VRFY', $string))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(array(250, 252)))) {
            return $error;
        }
    ...
    function noop()
    {
        if (is_pear_error($error = $this->_put('NOOP'))) {
            return $error;
        }
        if (is_pear_error($error = $this->_parseResponse(250))) {
            return $error;
