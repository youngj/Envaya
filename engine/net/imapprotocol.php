...
class Net_IMAPProtocol
{
    ...
    function cmdConnect($host= 'localhost', $port = 143)
    {
        ...
        if (is_pear_error($error = $this->_socket->connect($host, 
                                                           $port, 
                                                           null, 
                                                           $this->_timeout, 
        ...
        if (is_pear_error($this->_getRawResponse())) {
    ...
    function _send($data)
    {
        ...
        if (is_pear_error($error = $this->_socket->write($data))) {
    ...
    function _recvLn()
    {
        if (is_pear_error($this->lastline = $this->_socket->gets(8192))) {
            return new PEAR_Error('Failed to write to socket: ' 
    ...
    {
        $arguments = '';
        while (!is_pear_error($this->_recvLn())) {
    ...
    {
        if (is_pear_error($error = $this->_putCMD($cmdid,
                                                  'AUTHENTICATE',
                                                  'DIGEST-MD5'))) {
            return $error;
        }

        if (is_pear_error($args = $this->_recvLn())) {
            return $args;
        }
        ...
        if (is_pear_error($error = $this->_send("\r\n"))) {
            return $error;
        }
    }
    ...
    function cmdLogout()
    {
        ...
        if (is_pear_error($args = $this->_genericCommand('LOGOUT'))) {
            return $args;
        }
        if (is_pear_error($this->_socket->disconnect())) {
    ...
    function cmdSelect($mailbox)
    {
        ...
        if (!is_pear_error($ret = $this->_genericCommand('SELECT', 
                                                         $mailbox_name))) {
    ...
    function cmdAppend($mailbox, $msg, $flags_list = '', $time = '')
    {
        ...
        if ($this->hasCapability('LITERAL+') == true) {
            ...
            if (is_pear_error($error = $this->_recvLn())) {
                return $error;
            }

            if (is_pear_error($error = $this->_send($msg . "\r\n"))) {
                return $error;
            }
        }
    ...
    */
    ...
    {
        if (is_pear_error($res = $this->_genericCommand('STARTTLS'))) {
            return $res;
        }
        ...
        if (is_pear_error($res = $this->cmdCapability())) {
