...
class Net_IMAP extends Net_IMAPProtocol
{
    ...
    function connect($host, $port, $enableSTARTTLS = true)
    {
        ...
        if ($ret === true) {
            ...
            if ($this->hasCapability('STARTTLS') === true 
                ...
                if (is_pear_error($res = $this->cmdStartTLS())) {
                    return $res;
                }
            }
            return $ret;
    ...
    function login($user, $pass, $useauthenticate = true, $selectMailbox=true)
    {
        if ($useauthenticate) {
            ...
            if (is_pear_error($ret = $this->cmdAuthenticate($user, 
                                                            $pass, 
                                                            $method))) {
                ...
                if ($this->_serverAuthMethods == null 
                    ...
                    if ( is_pear_error($ret = $this->cmdLogin($user, $pass))) {
                        return $ret;
                    }
                } else {
                    return $ret;
        ...
        if ($selectMailbox) {
            //Select INBOX
            if (is_pear_error($ret = $this->cmdSelect($this->getCurrentMailbox()))) {
                return $ret;
            }
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
    ...
    {
        if ($expungeOnExit) {
            if (is_pear_error($ret = $this->cmdExpunge())) {
                return $ret;
            }
            if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
                $ret = $this->cmdLogout();
        ...
        if (is_pear_error($ret = $this->cmdLogout())) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    {
        if (is_pear_error($ret = $this->cmdSelect($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    {
        if (is_pear_error($ret = $this->cmdExamine($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    function getParsedHeaders($msg_id, 
                              ...
                              $uidFetch = false)
    {
        if (is_pear_error($ret = $this->getRawHeaders($msg_id, 
                                                      $part_id, 
                                                      $uidFetch))) {
            return $ret;
        }
    ...
    function getMessagesList($msg_id = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdFetch($message_set,
                                                 '(RFC822.SIZE UID)'))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
    ...
    function getNumberOfMessages($mailbox = '')
    {
        ...
        if (is_pear_error($ret = $this->cmdStatus($mailbox, 'MESSAGES'))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    function getNumberOfUnSeenMessages($mailbox = '')
    {
        ...
        if (is_pear_error($ret = $this->cmdStatus($mailbox, 'UNSEEN'))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret["RESPONSE"]["CODE"] 
    ...
    function getNumberOfRecentMessages($mailbox = '')
    {
        ...
        if (is_pear_error($ret = $this->cmdStatus($mailbox, 'RECENT'))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    function getStatus($mailbox = '')
    {
        ...
        if (is_pear_error($ret = $this->cmdStatus($mailbox, array('MESSAGES', 'RECENT', 'UIDNEXT', 'UIDVALIDITY', 'UNSEEN')))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    function getMailboxSize($mailbox = '')
    {
        ...
        if ($mailbox != '' && $mailbox != $this->getCurrentMailbox()) {
            ...
            if (is_pear_error($ret = $this->selectMailbox($mailbox))) {
                return $ret;
            }
        }
        ...
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            ...
            if ($mailbox != '' && $mailbox != $this->getCurrentMailbox()) {
                if (is_pear_error($ret = $this->selectMailbox($mailbox_aux))) {
                        return $ret;
                }
            }
                          ...
                          $uidCopy = false)
    {
        if ($source_mailbox == null) {
            ...
            if (is_pear_error($ret = $this->selectMailbox($source_mailbox))) {
                return $ret;
            }
        }
    ...
    {
        if (is_pear_error($ret = $this->cmdNamespace())) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    ******************************************************************/
    ...
    function getHierarchyDelimiter($mailbox = '')
    {
        /* RFC2060 says: 
            "the command LIST "" "" means get the hierachy delimiter:
             ...
             exist."
        */
        if (is_pear_error($ret = $this->cmdList($mailbox, ''))) {
            return $ret;
        }
                          ...
                          $returnAttributes = false)
    {
        ...
        if (is_pear_error($ret = $this->cmdList($reference, $mailbox))) {
            return $ret;
        }
    ...
    function mailboxExist($mailbox)
    {
        ...
        if (is_pear_error($ret = $this->getMailboxes($mailbox, true))) {
            return $ret;
        }
        if (count($ret) > 0) {
    ...
    {
        if (is_pear_error($ret = $this->cmdCreate($mailbox, $options))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    function deleteMailbox($mailbox)
    {
        ...
        if (is_pear_error($ret = $this->cmdDelete($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    {
        if (is_pear_error($ret = $this->cmdRename($oldmailbox, 
                                                  $newmailbox, 
                                                  $options))) {
            return $ret;
        }
    ...
    ******************************************************************/
    ...
    function subscribeMailbox($mailbox = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdSubscribe($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    function unsubscribeMailbox($mailbox = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdUnsubscribe($mailbox))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
                                     ...
                                     $returnAttributes = false)
    {
        ...
        if (is_pear_error($ret = $this->cmdLsub($reference, $mailbox))) {
            return $ret;
        }
    ...
    ******************************************************************/
    ...
    function getFlags($msg_id = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdFetch($message_set, 'FLAGS'))) {
            return $ret;
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    {
        if (is_pear_error($resp = $this->getFlags($message_nro))) {
            return $resp;
        }
        if (isset($resp[0])) {
            if (is_array($resp[0])) {
    ...
    ******************************************************************/
    ...
    function getStorageQuotaRoot($mailbox_name = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdGetQuotaRoot($mailbox_name))) {
    ...
    function getStorageQuota($mailbox_name = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdGetQuota($mailbox_name))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    {
        if (is_pear_error($ret = $this->cmdSetQuota($mailbox_name, $quota))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    {
        if (is_pear_error($ret = $this->cmdSetQuota($mailbox_name,
                                                    '',
                                                    $quota))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    ******************************************************************/
    ...
    function getACL($mailbox_name = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdGetACL($mailbox_name))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    {
        if (is_pear_error($ret = $this->cmdSetACL($mailbox_name, $user, $acl))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    {
        if (is_pear_error($ret = $this->cmdDeleteACL($mailbox_name, $user))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    function getMyRights($mailbox_name = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdMyRights($mailbox_name))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    function getACLRights($user,$mailbox_name = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdListRights($mailbox_name, $user))) {
            return new PEAR_Error($ret->getMessage());
        }
        if (strtoupper($ret['RESPONSE']['CODE']) != 'OK') {
            return new PEAR_Error($ret['RESPONSE']['CODE'] 
    ...
    ******************************************************************/
    ...
    function setAnnotation($entry, $values, $mailbox_name = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdSetAnnotation($mailbox_name, 
                                                         $entry, 
                                                         $values))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    function deleteAnnotation($entry, $values, $mailbox_name = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdDeleteAnnotation($mailbox_name, 
                                                            $entry, 
                                                            $values))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    function getAnnotation($entries, $values, $mailbox_name = null)
    {
        ...
        if (is_pear_error($ret = $this->cmdGetAnnotation($mailbox_name, 
                                                         $entries, 
                                                         $values))) {
            return new PEAR_Error($ret->getMessage());
        }
    ...
    *********************************************************/
    ...
    {
        if (is_pear_error($ret = $this->getMessages($msg_id, false))) {
            return $ret;
        }
